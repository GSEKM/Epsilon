	...
	private static class OpenDiagramAction extends Action {
		...
		*/
		public final void selectionChanged(IStructuredSelection selection) {
			...
			if (selection.size() == 1) {
				...
				if (selectedElement instanceof IotNavigatorItem) {
					selectedElement = ((IotNavigatorItem) selectedElement).getView();
				} else if (selectedElement instanceof IAdaptable) {
					selectedElement = ((IAdaptable) selectedElement).getAdapter(View.class);
				}
				if (selectedElement instanceof Diagram) {
					Diagram diagram = (Diagram) selectedElement;
					if (BoardEditPart.MODEL_ID.equals(IotVisualIDRegistry.getModelID(diagram))) {
						myDiagram = diagram;
					}
				}
			}
		...
		*/
		public void run() {
			...
			try {
				page.openEditor(editorInput, IotDiagramEditor.ID);
			} catch (PartInitException e) {
				IotDiagramEditorPlugin.getInstance().logError("Exception while openning diagram", e); //$NON-NLS-1$
			}
		}
