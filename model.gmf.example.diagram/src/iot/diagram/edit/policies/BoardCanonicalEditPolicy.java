 ...
 * @generated
 */
public class BoardCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	*/
	...
	protected List getSemanticChildrenList() {
		...
		List<IotNodeDescriptor> childDescriptors = IotDiagramUpdater.getBoard_1000SemanticChildren(viewObject);
		for (IotNodeDescriptor d : childDescriptors) {
	...
	*/
	protected void refreshSemantic() {
		...
		List<IotNodeDescriptor> childDescriptors = IotDiagramUpdater
				.getBoard_1000SemanticChildren((View) getHost().getModel());
		LinkedList<View> orphaned = new LinkedList<View>();
		// we care to check only views we recognize as ours and not shortcuts
		LinkedList<View> knownViewChildren = new LinkedList<View>();
		for (View v : getViewChildren()) {
			if (isShortcut(v)) {
				if (IotDiagramUpdater.isShortcutOrphaned(v)) {
					orphaned.add(v);
				}
				continue;
			}
		...
		for (Iterator<IotNodeDescriptor> descriptorsIterator = childDescriptors.iterator(); descriptorsIterator
				.hasNext();) {
			IotNodeDescriptor next = descriptorsIterator.next();
			String hint = IotVisualIDRegistry.getType(next.getVisualID());
		...
		for (IotNodeDescriptor next : childDescriptors) {
			String hint = IotVisualIDRegistry.getType(next.getVisualID());
	...
	*/
	private Collection<IAdaptable> refreshConnections() {
		...
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator.hasNext();) {
			...
			int diagramLinkVisualID = IotVisualIDRegistry.getVisualID(nextDiagramLink);
			...
			for (Iterator<IotLinkDescriptor> linkDescriptorsIterator = linkDescriptors
					.iterator(); linkDescriptorsIterator.hasNext();) {
				IotLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator.next();
	...
	private Collection<IotLinkDescriptor> collectAllLinks(View view, Domain2Notation domain2NotationMap) {
		...
		case BoardEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(IotDiagramUpdater.getBoard_1000ContainedLinks(view));
			}
	...
	*/
	private Collection<IAdaptable> createConnections(Collection<IotLinkDescriptor> linkDescriptors,
		...
		for (IotLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					IotVisualIDRegistry.getType(nextLinkDescriptor.getVisualID()), ViewUtil.APPEND, false,
